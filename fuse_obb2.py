...
def fuse_dets(dets):
    ...
    obb_angle = None
    for det in dets:
        if det.from_obb_detector:
            obb_angle = det.json["heading_angle"]
    for det in dets:
        if det.from_obb_detector:
            continue
        certainty = det.json["certainty"] if "certainty" in det.json else -1.0
        if obb_angle is not None:
            det.json["heading_angle"] = obb_angle
            result = Polygon(obb_to_pts(det.json))
...
def run(dst_folder, filename, base_paths):
    ...
    for model_idx, base_path in enumerate(base_paths):
        ...
        if base_path.find("073") != -1:
            print("Found1")
            for obj in objs:
                obj["width"] *= 0.938
        objs = [Obj(obj) for obj in objs]
        if base_path.find("exp7") != -1:
            print("Found2")
            for obj in objs:
                obj.from_obb_detector = True
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("python fuse_obb2.py dst_folder folder1 folder2 ...")
        exit(-1)
