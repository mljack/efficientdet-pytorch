...
def run(dst_folder, filename, base_paths):
    ...
    for model_idx, base_path in enumerate(base_paths):
        ...
        new_gt_objs = []
        info = info + f"[{len(objs)}] "
        if gt_objs is None:
            gt_objs = objs
        else:
            # Test AABB IoU with AABB groundtruth labels
            for obj in objs:
                max_IoU_obj, max_IoU = compute_max_IoU_obj(obj, gt_objs, IoU_obb)
                if max_IoU < 0.3:
                    new_gt_objs.append(obj)
            gt_objs += new_gt_objs
    info = info + f" => [{len(gt_objs)}] "
    print(info)

    gt_objs = [[obj.json] for obj in gt_objs]
    
    with open(dst_json_path, "w") as f:
        f.write(pprint.pformat(gt_objs, width=500, indent=1).replace("'", "\"").replace("True", "true").replace("False", "false"))
